...
1. **Audience reality.** The user base running cowork-aibos uses Slack, Discord, Telegram, Microsoft Teams, and combinations. Locking to Slack would force students on other platforms to fork the plugin or skip P02 entirely.
2. **Stack philosophy.** Tech stacks are liabilities, not assets. Locking the brief to one chat platform creates exactly the kind of vendor dependency cowork-aibos is meant to avoid.
...
**Mac users:** `/plugin update cowork-aibos` doesn't reliably work on Mac (Anthropic's open marketplace bugs). Mac users should download the new `cowork-aibos.zip` from the v0.6.0 release page and re-upload via Settings → Customize → Browse plugins. Their workspace files (`about-me/`, `projects/`, `outputs/`, `_aibos/`) survive the re-upload.

**Windows users:** `/plugin update cowork-aibos` works as expected.
