...
Confirm the plugin is at v0.7.0+. The presence of `skills/tidy-downloads/SKILL.md` in the plugin folder confirms v0.7.0+. If missing on Mac, instruct them to download the latest `cowork-aibos.zip` from https://github.com/AutomatedMarketer/cowork-aibos/releases/latest and re-upload via Settings → Customize → Browse plugins. On Windows, instruct them to run `/plugin update cowork-aibos`.
